...
void testApp::loadSettings() {
	...
	camWidth = xml.getValue("width", 1280);
	camHeight = xml.getValue("height", 720);
...
void testApp::draw() {
	...
	if(tracker.getFound()) {
		...
		ofDrawBitmapString("Mouth Width: "+ofToString((int) tracker.getGesture(ofxFaceTracker::MOUTH_WIDTH)), 60, 20);
		ofDrawBitmapString("Mouth Height: "+ofToString((int) tracker.getGesture(ofxFaceTracker::MOUTH_HEIGHT)), 60, 40);
		ofDrawBitmapString("Jaw Openness: "+ofToString((int) tracker.getGesture(ofxFaceTracker::JAW_OPENNESS)), 60, 60);
		ofDrawBitmapString("Right Eyebrow Height: "+ofToString((int) tracker.getGesture(ofxFaceTracker::RIGHT_EYEBROW_HEIGHT)), 60, 80);
		ofDrawBitmapString("Left Eyebrow Height: "+ofToString((int) tracker.getGesture(ofxFaceTracker::LEFT_EYEBROW_HEIGHT)), 60, 100);
		ofDrawBitmapString("Left Eye Openness: "+ofToString((int) tracker.getGesture(ofxFaceTracker::LEFT_EYE_OPENNESS)), 60, 120);
		velocity=100;
		//note=(int)ofMap(tracker.getGesture(ofxFaceTracker::MOUTH_HEIGHT),0,12,60,84);
		//note=60;
		//midiOut.sendNoteOn(channel, note,  velocity);
		//int par_val=(int)ofMap(tracker.getGesture(ofxFaceTracker::MOUTH_HEIGHT),0,12,0,127);
		//midiOut.sendProgramChange(channel,par_val);
		int par_val=(int)ofMap(tracker.getGesture(ofxFaceTracker::MOUTH_HEIGHT),0,14,200,16380);
		midiOut.sendPitchBend(channel,par_val);
